    ...
    return (
        <>
            ...
            <nav className={`lg:flex  gap-x-6 transform transition ${isOpen ? 'flex flex-col px-8 py-12 z-50  gap-y-6 absolute top-0 bg-white right-0  translate-x-0 ' : 'hidden max-md:translate-x-full'}`}>
                ...
                <button className="block lg:hidden" onClick={() => setOpen(!isOpen)}>
                    <svg
                        className={`fill-current h-3 w-3 ${isOpen ? "block" : "hidden"} z-50 absolute top-5 right-2`}
                        viewBox="0 0 20 20"
                        xmlns="http://www.w3.org/2000/svg"
                    >
            ...
            <Link href="/contact" className="items-center hidden px-5 py-2 font-medium text-gray-800 bg-white border border-gray-600 rounded-full shadow lg:inline-flex hover:bg-gray-100" style={{ fontFamily: 'Open Sans', fontSize: 'large' }}>
